...
public class AIMark2 extends StellarObject{
	...
	private AIMark2(Physics phys, StellarObject target){
		super(phys);
		this.target = target;
		phys.getCImg().setColor(Color.red);
	}

	public static AIMark2 makeAI(float x, float y, StellarObject target){
		//make centroid
		Vector2f centroid = new Vector2f(x, y);
		//make cimg
		_AIMark2Collider ac = new _AIMark2Collider();
		...
		//make the AIMark2
		AIMark2 ai = new AIMark2(new _AIMark2Physics(centroid, cimg, gimg), target);
	...
	Vector2f wander(float magnitude){
		...
		return new Vector2f(rx,ry).normalise().scale(rand.nextFloat()*magnitude);
	}
	...
	private static class _AIMark2Physics extends Physics{
		
		private static final float FRICTION_DEFAULT = .995f;

		public _AIMark2Physics(
			Vector2f centroid,
			CollisionImage cimg,
			GraphicalImage gimg
			){
	...
	private static class _AIMark2Collider extends Collider{
		public AIMark2 ai;
	}
